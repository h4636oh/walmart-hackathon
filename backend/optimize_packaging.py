...
class Block:
    def __init__(self, id, length: int, width: int, height: int, weight, 
                 customer_id, fragility=0, priority=0, orientations=[]):
        self.id = id
        ...
        self.allowed_orientations = sorted(possible_orientations, key=lambda x: x[0] * x[1])
...
class position:
    def __init__(self, box, x, y, z):
        ...
        self.y = y
        self.z = z
        self.center_of_gravity = (x + box.dimensions[0]/2, 
...
class System:
    ...
    def initial_feasibility(self):
        ...
        return self.total_weight <= self.max_weight and total_volume <= math.prod(self.container_dims)
    ...
    def constructive_packing(self, blocks, visualize=False):
        ...
        def fits_in_container(orientation, x, y, z):
            return (x + orientation[0] <= self.container_dims[0] and
                    y + orientation[1] <= self.container_dims[1] and
                    z + orientation[2] <= self.container_dims[2])
        ...
        def find_best_position(block):
            for x, y, z in sorted(potential_points, key=lambda p: (p[2], p[0]+p[1])):  # Prefer lower z first
                for orientation in block.allowed_orientations:
                    if (fits_in_container(orientation, x, y, z) and
                        not overlap(orientation, x, y, z) and
                        has_sufficient_support(orientation, x, y, z)):
    ...
    def evaluate(self, solution):
        def check_Center_of_Gravity():
            ...
            if (com_x < safe_x or com_x > self.container_dims[0] - safe_x or
                com_y < safe_y or com_y > self.container_dims[1] - safe_y):
                solution.score = 0
    ...
    def sort_solutions(self, solutions):
        def sort_key(sol):
            placed_volume = sum(p.placed_box.volume for p in sol.placement) * 100 / self.total_weight
            com_x = sum(p.placed_box.weight * p.x for p in sol.placement) / self.total_weight
            com_y = sum(p.placed_box.weight * p.y for p in sol.placement) / self.total_weight
            center_x, center_y = self.container_dims[0]/2, self.container_dims[1]/2
            stability_score = ((com_x - center_x)**2 + (com_y - center_y)**2) ** 0.5
            return (placed_volume, -stability_score)  # Maximize volume, minimize stability score
    ...
    def visualize_3d(self, solution):
        ...
        for pos in solution.placement:
            ...
            self.ax.text(x + l/2, y + w/2, z + h/2, str(box.id), 
        ...
        self.ax.set_box_aspect([self.container_dims[0], 
        ...
        self.ax.text2D(0.05, 0.95, f"Utilization: {utilization:.1f}%", 
...
def Optimizer(boxes, container_dims, max_weight=0, zone_range=None, zone_weights=None, visualize=False):
    if max_weight < 0:  
